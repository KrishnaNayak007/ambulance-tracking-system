// hospitals.js
export const odishaHospitals = [
  "AIIMS Bhubaneswar",
  "SCB Medical College, Cuttack",
  "KIMS, Bhubaneswar",
  "SUM Hospital, Bhubaneswar",
  "Hi-Tech Medical College, Bhubaneswar",
  "Capital Hospital, Bhubaneswar",
  "VIMSAR, Burla",
  "MKCG Medical College, Berhampur",
  "Balasore District Hospital",
  "Sundargarh District Hospital",
  "Keonjhar District Hospital",
  "Jagatsinghpur District Hospital",
  "Puri District Hospital",
  "Cuttack District Hospital",
  "Bolangir District Hospital",
  "Bhadrak District Hospital"
];
